 ...
 * @param {org.clp.citi.EarnPoints} earnPoints
 * @transaction
 */
async function EarnPoints(earnPoints) {
  ...
  //update member
  const memberRegistry = await getParticipantRegistry('org.clp.citi.Customer');
  await memberRegistry.update(earnPoints.member);

  // check if partner exists on the network
  const partnerRegistry = await getParticipantRegistry('org.clp.citi.Partner');
 ...
 * @param {org.clp.citi.UsePoints} usePoints
 * @transaction
 */
